...
"""
MCP客户端管理器的开发思路：
1.在Agen执行的过程中，有可能需要调用多次工具，
    但是因为MCP工具的每次获取都需要调用客户端会话的list_tools()方法
    非常耗时，所以需要我们缓存工具的参数信息，只有在初始化的时候才调用一次，
    并且在摧毁MCP客户端管理器的时候一并清除；
2.在前端UI交互中，无论MCP服务器是否启动，都会显示工具列表消息，
...
4.另外有可能有一些环境变量是存储在我们整个系统中的，在初始化MCP服务的时候，需要将传递进来
    缓急变量与系统的环境变量进行合并后传递给MCP服务；
5.使用AsyncExitStack异步上下文管理器来管理上下文，避免使用with多层嵌套；
6.MCPClientManager的初始化非常耗时，所以需要有机制可以判定避免重复初始化；
